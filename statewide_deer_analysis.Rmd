...
```{r MultispeciesMD, eval=FALSE}
multispecies_md <- prepare_model_data_multispecies(species = "All deer",
...
saveRDS(multispecies_md, "data/multispecies_md.rds")

multispecies_md_fit <- model_poisson_ms_co$sample(data = multispecies_md,
                                         chains = 2,
                                 parallel_chains = 2, 
                                 refresh = 10, init = 0.1,
                                 max_treedepth = 10,
...
multispecies_md_fit$save_object("outputs/models/multispecies_poisson_co.rds")
```
...
terra::values(pred_raster_means[[i]])[!is.na(terra::values(pred_raster_means[[i]]))] <- apply(gp_preds_draws_all[[i]], 1, mean, na.rm = T, trim = 0.25)
