...
# multispecies_data$site_EVCGROUP <- NULL
...
rn_dens <- model$summary("N_site", prob_occ = ~ mean(. > 0), trimmed_mean = ~mean(., trim = 0.01), posterior::default_summary_measures(), posterior::default_convergence_measures())
...
abundance_table <- function(model, regions, pred_area, caption, species_index) {
  ...
  reg <- model$summary("Nhat_reg", prob_occ = ~ mean(. > 0), trimmed_mean = ~mean(., trim = 0.01), posterior::default_summary_measures(), posterior::default_convergence_measures())
  ...
  regional_abundance <- reg[which_sp,] %>%
    dplyr::bind_rows(model$summary("Nhat", prob_occ = ~ mean(. > 0), trimmed_mean = ~mean(., trim = 0.01), posterior::default_summary_measures(), posterior::default_convergence_measures())[species_index,]) %>% 
