...
ab_formula_4 <- ~ scale(sqrt(BareSoil)) + scale(sqrt(Nitrogen)) + scale(log(PastureDistance)) + scale(BIO15) + scale(sqrt(ForestEdge)) + scale(TreeDensity) + scale(TreeDensity^2)
...
multispecies_data_f <- prepare_model_data_multispecies(species = c("Cervus unicolor", 
                                                                 "Dama dama", 
                                                                 "Cervus elaphus"),
                     ...
                     hs_scale_global = 1, 
                     hs_scale_slab = 0.5,
                     hs_df_slab = 4, filter_behaviour = FALSE)
...
saveRDS(multispecies_data, "data/multispecies_data_evc_group.rds")
} else {
...
model_negbin_ms_zip_evc <- cmdstan_model(here::here("stan", "count_det_nondet_negbin_ms_zip_evc.stan"))

model_negbin_ms <- cmdstan_model(here::here("stan", "count_det_nondet_negbin_ms.stan"))
model_negbin_ms2 <- cmdstan_model(here::here("stan", "count_det_nondet_negbin_ms2.stan"))
```
...
multispecies_model_negbin <- model_negbin_ms$sample(data = multispecies_data, 
                                                chains = 6,
                                 parallel_chains = 6, 
                                 init = 0.1, 
                                 max_treedepth = 10, 
...
multispecies_model_negbin2 <- model_negbin_ms2$sample(data = multispecies_data, 
                                                chains = 4,
                                 parallel_chains = 4, 
                                 init = 0.1, 
                                 max_treedepth = 10, 
...
multispecies_model_negbin_zip_evc$save_object("outputs/models/multispecies_model_negbin_zip_evc.rds")
```
...
for(i in 1:length(funs)) {
ppc_red[[i]] <- posterior_checks_multispecies(model = multispecies_model_negbin_zip_evc, 
