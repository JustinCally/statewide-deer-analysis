...
```{r ModelSettings}
# STAN settings
ni <- 500 # sampling iterations
nw <- 500 # warmup iterations 
...
titles <- c("Max", "Mean", "SD", "Average Site Counts")
...
```{r DistancePlots, fig.caption="Distance-sampling detection process for Sambar Deer (group size = 1). Herbaceous understorey cover in the area surrounding the camera impacts detection probability."}
...
det_curve <- model_fits[[2]]$draws("DetCurve", format = "draws_matrix") %>%
  as.data.frame() %>%
  head(250) %>%
...
y_combined <- colSums(model_data[[2]]$y[,,1]) %>% # just for group size 1
  as.data.frame() %>%
  `colnames<-`("Count") %>%
...
```{r bioregions, fig.height=24, fig.width = 5}
...
```{r FixedEffects, fig.height=10, fig.width = 14, warning = FALSE, message = FALSE}
ab_joined_list <- list()
