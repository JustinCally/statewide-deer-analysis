...
# Projects to select.
project_short_name <- c("hog_deer_2023", "StatewideDeer", "budj_bim_pig_deer_2022")
...
saveRDS(multispecies_data, "data/multispecies_data_bb.rds")
} else {
...
model_negbin_ms_hp <- cmdstan_model(here::here("stan", "count_det_nondet_negbin_ms_hp.stan"))
```
...
values(mean_raster_discrete[[i]]) <- cut(values(mean_raster_discrete[[i]]) , 
                                         breaks = c(0, 0.1,1,3,5,10,15,max_pred), 
...
We can generate detection function plots that show how detection rates fall-off over varying distances.

```{r DistancePlots, fig.caption="Distance-sampling detection process for Sambar Deer (group size = 1). Herbaceous understorey cover in the area surrounding the camera impacts detection probability.", fig.width = 8, fig.height = 6, message = FALSE, warning=FALSE}
...
scaleHN <- det_curve_sum %>% filter(Distance == 1.5) %>% pull(median) %>% mean()
...
y_combined <- colSums(rowSums(multispecies_data$y, dims = 2)) %>% # just for group size 1
  ...
  mutate(Distance = midpts,
         ...
         PropS = scaleHN*CountS/(max(CountS)))
...
```{r detectionplots, fig.width=8, fig.height=5, fig.cap="Detection Probability for the various methods of survey. Camera trap detection probability is based on average deployment length, while transects are based on 3 independent transect searches", warnings = FALSE, message = FALSE}
...
bform1 <- bf(mvbind(sqrt(CanopyCov), sqrt(Saplings), sqrt(Seedlings), sqrt(BGroundCover)) ~ sqrt(AllDeerDensity) + (1|Bioregion)) +
  set_rescor(TRUE)

bform2 <- bf(ExoticPresence ~ sqrt(AllDeerDensity) + (1|Bioregion))

fit1 <- brm(bform1, data = site_density_df_joined, chains = 2, cores = 2)
fit2 <- brm(bform2,  data = site_density_df_joined, chains = 2, cores = 2, family = bernoulli(link = "logit"))
```
