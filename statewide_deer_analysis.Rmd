...
```{r ModelSettings}
# STAN settings
ni <- 400 # sampling iterations
nw <- 400 # warmup iterations 
...
multispecies_model_zip$save_object("outputs/models/multispecies_zip_2.rds")
```

```{r}
multispecies_model_zip_hs <- model_poisson_ms_zip_hs$sample(data = multispecies_data, 
                                                chains = 6,
                                 parallel_chains = 6, 
                                 init = 0.1, 
                                 max_treedepth = 10, 
...
ppc_sambar[[i]] <- posterior_checks_multispecies(model = multispecies_models[[1]], 
