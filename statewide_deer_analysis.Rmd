...
multispecies_data <- prepare_model_data_multispecies(species = c("Cervus unicolor", 
                     ...
                     hs_scale_global = 1, 
                     hs_scale_slab = 0.5,
                     hs_df_slab = 4, filter_behaviour = TRUE)
...
We generate a set of models to evaluate against one another:
...
for(i in 1:nrow(models_to_run)) {
  ...
  data_to_use$X_psi <- data_to_use$X_psi[,params_to_use]
  data_to_use$X_pred_psi <- data_to_use$X_pred_psi[,params_to_use]
  data_to_use$m_psi <- length(params_to_use)
...
ppc_sambar[[i]] <- posterior_checks_multispecies(model = multispecies_model_negbin, 
...
for(i in 1:length(funs)) {
ppc_fallow[[i]] <- posterior_checks_multispecies(model = multispecies_model_negbin, 
...
for(i in 1:length(funs)) {
ppc_red[[i]] <- posterior_checks_multispecies(model = multispecies_model_negbin, 
...
sambar_preds <- site_preds(multispecies_model_negbin, cams_curated = cams_curated, species_index = 1)
fallow_preds <- site_preds(multispecies_model_negbin, cams_curated = cams_curated, species_index = 2)
red_preds <- site_preds(multispecies_model_negbin, cams_curated = cams_curated, species_index = 3)
...
gp_preds_draws_all <- multispecies_model_negbin$draws("pred", format = "matrix")
...
abundance_table(multispecies_model_negbin, regions = reg, pred_area = table(multispecies_data$pred_reg), caption = "Regional estimates of Sambar Deer Density", species_index = 1)
abundance_table(multispecies_model_negbin, regions = reg, pred_area = table(multispecies_data$pred_reg), caption = "Regional estimates of Fallow Deer Density", species_index = 2)
abundance_table(multispecies_model_negbin, regions = reg, pred_area = table(multispecies_data$pred_reg), caption = "Regional estimates of Red Deer Density", species_index = 3)
```
...
det_rates <- multispecies_model_negbin$summary("p") %>%
...
all_draws <- multispecies_model_negbin$draws("beta_trans_det", format = "df")
