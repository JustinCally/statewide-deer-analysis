...
model_negbin_ms_co <- cmdstan_model(here::here("stan", "count_only_negbin_ms2.stan"))
...
# reg <- vicspatial::vicmap_query("open-data-platform:delwp_region") %>%
#     vicspatial::collect() %>%
#   sf::st_transform(3111) %>%
#   sf::st_simplify(dTolerance = 250) 
state <- vicspatial::vicmap_query("open-data-platform:vmlite_victoria_polygon_su5") %>%
...
plot_threshold <- function(raster, state, crop = NULL) {
  ...
  plot <- ggplot2::ggplot() +
    ...
    ggspatial::annotation_scale() +
    ggplot2::geom_sf(data = state, alpha = 0.1, linewidth = 0.5, fill = "grey98") + 
    delwp_theme()
...
Below we show the average detection rate for a given group of deer in front of the camera (up to 12.5m). The average group sizes for the four species `r paste(round(model_fits[[top]]$summary("av_gs")$mean, 2), collapse = ", ")`.
...
bioreg <- vicspatial::vicmap_query("open-data-platform:vbioreg100") %>%
    vicspatial::collect() %>%
