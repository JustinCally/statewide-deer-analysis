...
model_negbin_ms_zip <- cmdstan_model(here::here("stan", "count_det_nondet_negbin_ms_zip.stan"))
model_poisson_co_ms_zip <- cmdstan_model(here::here("stan", "count_only_poisson_ms_zip.stan"))
model_poisson_ms_zip_re <- cmdstan_model(here::here("stan", "count_det_nondet_poisson_ms_zip_re.stan"))
# model_poisson_ms_zip_hs <- cmdstan_model(here::here("stan", "count_det_nondet_poisson_ms_zip_hs.stan"))
```
...
saveRDS(multispecies_data, "data/multispecies_data_4.rds")


multispecies_models <- list()

for(i in 1:2) {
  
multispecies_data$keyfun <- i - 1

multispecies_models[[i]] <- model_poisson_ms$sample(data = multispecies_data, 
                                                chains = nc,
                                 parallel_chains = nc, 
                                 init = 0.1, 
                                 max_treedepth = 10, 
...
multispecies_models[[i]]$save_object(paste("outputs/models/multispecies_",i,".rds"))
...
multispecies_co_zip <- model_poisson_co_ms_zip$sample(data = multispecies_data, 
                                                chains = 2,
                                 parallel_chains = 2, 
                                 init = 0.1, 
                                 max_treedepth = 10, 
...
# multispecies_model_zip_co <- model_poisson_co_ms_zip$sample(data = multispecies_data, 
#                                                 chains = 8,
#                                  parallel_chains = 8, 
#                                  init = 0.1, 
#                                  max_treedepth = 10, 
...
funs <- c(max, mean, sd, ppc_scatter_avg, prop_zero, sd_90)
titles <- c("Max", "Mean", "SD", "Average Site Counts", "Proportion Zeros", "SD 90 %")
...
for(i in 1:length(funs)) {
ppc_fallow[[i]] <- posterior_checks_multispecies(model = multispecies_models[[1]], 
                 model_data = multispecies_data, species_index = 2,
                 stat = funs[[i]], 
                 integrated = F, 
                 title = titles[i])
...
for(i in 1:length(funs)) {
ppc_red[[i]] <- posterior_checks_multispecies(model = multispecies_models[[1]], 
...
gp_preds_draws_all <- multispecies_model_zip_co$draws("pred", format = "matrix")

for(i in 1:length(deer_species_all)) {

which_sp <- which(stringr::str_detect(string = colnames(gp_preds_draws_all),
                                         pattern = paste0("pred\\[", species_index)))
