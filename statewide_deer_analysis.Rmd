...
deer_predictions <- list(Sambar = sambar_preds, 
                         Fallow = fallow_preds, 
                         Red = red_preds, 
                         Hog = hog_preds)
...
From our point density estimates of deer, we can model relationships between deer densities at sites and a range of structural vegetation measures collected during the surveys. For this part of the analysis we implement two different models to investigate the relationship between vegetation and vegetation.  

Firstly we construct a multivariate gaussian model, where several response variables are continuous data measured at sites. They are:  
...
These response variables have square-root transformations applied to allow them to be on a more normal scale,. Secondly we run a model with a multivariate response and a binomial logit-link. This allows us to model the relationship between deer densities and the following binomial reponse varibles:  

+ Presence of exotic flora  
...
bform1 <- bf(mvbind(sqrt(CanopyCov), 
                    sqrt(BGroundCover), 
                    sqrt(NWUCover), 
                    sqrt(NNWHUCover), 
                    sqrt(TopHeight)) ~ sqrt(AllDeerDensity) + (1|EVC)) + set_rescor(FALSE) 

bform2 <- bf(mvbind(ExoticPresence, SeedlingsPresence, SaplingsPresence) ~ sqrt(AllDeerDensity) + (1|EVC))
...
sjPlot::tab_model(fit1, show.re.var = F)
sjPlot::tab_model(fit2, show.re.var = F)
```
