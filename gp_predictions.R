...
ab_formula <- ~ scale(BIO12)  + scale(BIO04)  + scale(BIO01) + scale(TreeDensity) + scale(sqrt(PastureDistance)) + scale(TWIND) + scale(BIO15) + scale(SLOPE) + scale(MRVBF) + scale(ForestEdge) + scale(BareSoil) + scale(NonPhotosyntheticVeg) + scale(sqrt(SLOPE) * BIO04)
...
pred_grid <- vic_model_data_resampled_add

pred_grid # prediction grid including covariates
OS<- na.omit(as.vector(terra::cellSize(pred_grid, mask = T))/1e6)    # offset (cell size)
Xpred<- ab_model_pred_matrix  # Fixed effects

obs_distances <- calc_point_distances(coords)
pred_distances <- dist(coords_pred_scaled, upper=TRUE, diag=TRUE)
# pred_distances <- calc_point_distances(coords_pred_scaled)
obs_to_pred_distances <- calc_point_distances(coords, coords_pred_scaled)
...
gp_preds <- bettermc::mclapply(X = as.list(seq_len(nsims)),
                     FUN = predict_gp, obs_distances = obs_distances,
                       ...
                       Xpred = Xpred,
                       OS = OS,
                       kernel = "quad", mc.progress = TRUE,
                       mc.cores = 1)

# plan(sequential)

gp_preds<- do.call(rbind, gp_preds)
