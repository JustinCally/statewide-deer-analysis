...
transformed parameters {
  ...
  array[S, n_site, n_gs] real lp_count;
  ...
  array[S, n_site] real<lower=0, upper=1> phi;
  ...
  for (s in 1:S) {
    ...
    f[s] = gp_inv_rfft2(z_std[s] * gp_sigma[s], rep_matrix(beta_occ[s], num_rows, num_cols), rfft2_cov[s]);
    for (n in 1:n_site) {
      f_site[s, n] = f[s, sampled_row[n], sampled_col[n]];
      phi[s, n] = inv_logit(f_site[s, n]);
    }
  }
...
for(n in 1:n_site) {
  ...
  for(j in 1:n_gs) {
    ...
    for(s in 1:S) {
    lambda[s,n,j] = exp(log_lambda_psi[s,n] + log_p[n,j] + log_activ + log(eps_ngs[s,n,j])) * survey_area[n];  // cap log-lambda at 15
    }
  }
}
// Occupancy formulation for imperfect detection using detection history y2
for (s in 1:S) {
  for (n in 1:n_site) {
    if (n_survey[n] > 0) {
      int n_tr = end_idx[n] - start_idx[n] + 1;
      vector[n_tr] r_sub = r[start_idx[n]:end_idx[n]];
      array[n_tr] int y2_sub = y2[s, start_idx[n]:end_idx[n]];

      // Probability of at least one detection if site is occupied
      real p_det_occ = 1 - prod(1 - r_sub);

      if (sum(y2_sub) == 0) {
        // All detections are 0
        lp_site[s,n] = log_sum_exp(
          bernoulli_lpmf(0 | phi[s,n]),  // unoccupied
          bernoulli_lpmf(1 | phi[s,n]) + bernoulli_lpmf(0 | p_det_occ) // occupied but undetected
        );
      } else {
        // At least one detection
        lp_site[s,n] = bernoulli_lpmf(1 | phi[s,n]) +
                       bernoulli_lpmf(1 | p_det_occ);  // occupied and detected
      }
    } else {
      lp_site[s,n] = 0;
    }
  ...
  // Evaluate ZINB lp_count likelihood per group size
  for (n in 1:n_site) {
    for (j in 1:n_gs) {
      for (s in 1:S) {
        if (n_obs[n,j,s] == 0) {
          lp_count[s,n,j] = log_sum_exp(
            bernoulli_lpmf(0 | phi[s,n]),
            bernoulli_lpmf(1 | phi[s,n]) + poisson_lpmf(0 | lambda[s,n,j])
          );
        } else {
          lp_count[s,n,j] = bernoulli_lpmf(1 | phi[s,n]) + poisson_lpmf(n_obs[n,j,s] | lambda[s,n,j]);
        }
      }
    }
  }
...
model {
  ...
  // In model block (ZINB likelihood)
// ZINB combo penalty:
for (s in 1:S) {
  for (n in 1:n_site) {
    for (j in 1:n_gs) {
      // soft penalty on log(expected count) = log(lambda * phi)
      real log_expectation = log(lambda[s,n,j] + 1e-8) + log(phi[s,n] + 1e-8);
      target += normal_lpdf(log_expectation | log(5), 1.5);  // Prior center: log(5) ≈ 1.6
    }
  }
}
...
generated quantities {
  ...
  for (s in 1:S) {
    eps_gs_ave[s] = softmax(zeta[s]);
    av_gs[s] = dot_product(gs, eps_gs_ave[s]);
    trunc_counter[s] = 0;
  }
  ...
  for (n in 1:n_site) {
    for (j in 1:n_gs) {
      ...
      for (s in 1:S) {
        log_lik2[s,n,j] = lp_count[s,n,j];
        n_obs_true[s,n,j] = gs[j] * poisson_rng(lambda[s,n,j]) * bernoulli_rng(phi[s,n]);
        n_obs_pred[s,n,j] = gs[j] * poisson_rng(lambda[s,n,j]) * bernoulli_rng(phi[s,n]);
      }
      log_lik2_site[n,j] = log_sum_exp(log_lik2[,n,j]);
    }

    log_lik_det[n] = log_sum_exp(log_lik1[n,]);
    log_lik[n] = log_sum_exp(log_sum_exp(log_lik_det[n], log_sum_exp(log_lik2_site[n,])), log_sum_exp(lp_site[,n]));

    for (s in 1:S) {
      log_lik2_species[s,n] = log_sum_exp(log_lik2[s,n,]) + lp_site[s,n];
    ...
    if (keyfun == 0) {
      for (j in 0:max_int_dist) {
        DetCurve[n,j+1] = exp(-(j + 0.5)^2 / (2 * square(sigma[n])));
      }
    } else if (keyfun == 1) {
      for (j in 0:max_int_dist) {
  ...
  for (s in 1:S) {
    ...
    for (i in 1:npc) {
      pred[s,i] = poisson_rng(exp(X_pred_psi[i,] * beta_psi[s])) * prop_pred[i] * av_gs[s] * bernoulli_rng(inv_logit(f[s, prediction_row[i], prediction_col[i]]));
