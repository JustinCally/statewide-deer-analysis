...
parameters {
 // abundance parameters
  ...
  // site RE
  array[S] real<lower=0> site_sd_lambda;
  array[S] vector[n_site] site_raw_lambda;
  // bioregion RE ZIP
...
transformed parameters {
  ...
  array[S] vector[n_site] eps_site_lambda; // bioregion random effect
...
for(n in 1:n_site) {
  ...
  for(s in 1:S) {
  eps_site_lambda[s,n] = site_sd_lambda[s] * site_raw_lambda[s,n];
  // define log lambda
  logit_phi[s,n] = eps_bioregion_zip[s,site_bioreg[n]];
  phi[s,n] = inv_logit(logit_phi[s,n]);

  log_lambda_psi[s,n] = X_psi[n,] * beta_psi[s] + eps_site_lambda[s,n]; // + eps_bioregion[s,site_bioreg[n]];
...
if (n_survey[n] > 0) {
    if(any_seen[s,n] == 0){ // not seen
      lp_site[s, n] = log_sum_exp(bernoulli_lpmf(0 | phi[s,n]),
      bernoulli_lpmf(1 | phi[s,n]) +
...
model {
  for(s in 1:S) {
    ...
    site_sd_lambda[s] ~ student_t(4, 0, 1);
    site_raw_lambda[s,] ~ normal(0, 1);
    // bioregion RE
