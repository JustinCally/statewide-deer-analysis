...
data {
  ...
  // transect information for RN component
  int<lower=1> trans;
  ...
  array[n_site, S] int<lower=1> n_max;  // max N for RN
...
parameters {
  ...
  real odRNmu;           // global baseline for RN abundance dispersion
}
...
transformed parameters {
  ...
  array[S, n_site] real lp_site;          // RN log-lik per species & site
  ...
  for (n in 1:n_site) {
    ...
    for (s in 1:S) {
      ...
      // Royle-Nichols p-bar (not used later; removed for efficiency) – skipped

      // RN log-likelihood at this site for species s
      if (n_survey[n] > 0) {
        int a = start_idx[n];
        int b = end_idx[n];
        ...
        if (b >= a && a >= 1) {
          ...
          if (any_seen[s, n] == 0) {
            lp[1] = log_sum_exp(
              neg_binomial_2_log_lpmf(0 | log_lambda_psi[s][n], odRN),
              neg_binomial_2_log_lpmf(1 | log_lambda_psi[s][n], odRN)
              + bernoulli_lpmf(y2[s, a:b] | r[a:b])
            );
          } else {
            lp[1] = neg_binomial_2_log_lpmf(1 | log_lambda_psi[s][n], odRN)
                    + bernoulli_lpmf(y2[s, a:b] | r[a:b]);
          }

          // k >= 2
          for (k in 2:K) {
            // detection prob for at least one detection out of k individuals
            // 1 - (1 - r)^k
            lp[k] = neg_binomial_2_log_lpmf(k | log_lambda_psi[s][n], odRN)
                    + bernoulli_lpmf(y2[s, a:b] | 1 - pow(1 - r[a:b], k));
          }
...
generated quantities {
  ...
  for (n in 1:n_site) {
    ...
    log_lik[n] = log_lik_det[n] + lp_counts + lp_rn;
    ...
    for (s in 1:S) {
      ...
      log_lik2_species[s, n] = lp_counts_sn + lp_site[s, n];
