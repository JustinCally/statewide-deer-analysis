...
transformed parameters {
  // re
  vector[np_reg] eps_region; // bioregion random effect
...
// define log lambda
  ...
  phi[n] = exp(phi_int + eps_region[site_reg[n]]);
...
model {
  ...
  // alpha ~ normal(0, 2);
  region_mu ~ normal(0,2);
  region_sd ~ normal(0, 1);
  region_raw ~ std_normal();
  ...
  target += log_sum_exp(log(1-r[start_idx[n]]),
                            log1m(1-r[start_idx[n]]) + neg_binomial_2_lpmf(n_obs[n,j] | lambda[n,j], phi[n]));
    } else {
      target += log1m(1-r[start_idx[n]]) + neg_binomial_2_lpmf(n_obs[n,j] | lambda[n,j], phi[n]);
    }
...
for(i in 1:npc) {
  pred[i] = neg_binomial_2_rng(exp(X_pred_psi[i,] * beta_psi + eps_bioregion[pred_bioreg[i]]), exp(phi_int + eps_region[pred_reg[i]])) * prop_pred[i]; //offset
}
Nhat = sum(pred);
}
