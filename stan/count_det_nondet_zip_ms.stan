...
transformed parameters {
  ...
  for (s in 1:S) {
    ...
    for (n in 1:n_site) {
      ...
      phi[s, n] = inv_logit(eps_bioregion[s,site_bioreg[n]]);
    }
  }
...
generated quantities {
  ...
  for (s in 1:S) {
    ...
    for (i in 1:npc) {
      pred[s,i] = poisson_rng(exp(X_pred_psi[i,] * beta_psi[s] + f[s, prediction_row[i], prediction_col[i]])) * prop_pred[i] * av_gs[s] * bernoulli_rng(inv_logit(eps_bioregion[s, pred_bioreg[i]]));
