...
model {
  ...
  for(j in 1:n_gs) {
  target += neg_binomial_2_lpmf(n_obs[n,j] | lambda[n,j], phi);
...
for(n in 1:n_site) {
  ...
  n_obs_true[n, j] = gs[j] * (neg_binomial_2_rng(exp(log_lambda_psi[n] + log(eps_ngs[n,j])), phi));
  n_obs_pred[n,j] = gs[j] * neg_binomial_2_rng(lambda[n,j], phi);
    }
...
for(i in 1:npc) {
  pred[i] = neg_binomial_2_rng(exp(X_pred_psi[i,] * beta_psi + eps_bioregion[pred_bioreg[i]]), phi) * prop_pred[i] * av_gs; //offset
