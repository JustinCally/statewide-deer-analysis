...
parameters {
 // abundance parameters
  ...
  // od
  array[S] real od_mu;
  array[S] matrix[num_rows, num_cols] z;
...
transformed parameters {
  ...
  for (s in 1:S) {
    ...
    f[s] = gp_inv_rfft2(z[s], rep_matrix(0, num_rows, num_cols), rfft2_cov[s]);
    for (n in 1:n_site) {
      ...
      for (s in 1:S) {
        if (n_obs[n,j,s] == 0) {
          lp_count[s,n,j] = log_sum_exp(
            ...
            bernoulli_lpmf(1 | phi[s,n]) + neg_binomial_2_lpmf(0 | lambda[s,n,j], od[s])
          );
        } else {
          lp_count[s,n,j] = bernoulli_lpmf(1 | phi[s,n]) + neg_binomial_2_lpmf(n_obs[n,j,s] | lambda[s,n,j], od[s]);
        }
      }
    }
  }
...
model {
  ...
  for(s in 1:S) {
    ...
    gp_sigma[s] ~ gamma(1, 1);
    log_length_scale[s] ~ normal(0, 1);
  }
...
generated quantities {
  ...
  for (n in 1:n_site) {
    for (j in 1:n_gs) {
      ...
      for (s in 1:S) {
        ...
        n_obs_true[s,n,j] = gs[j] * neg_binomial_2_rng(lambda[s,n,j], od[s]) * bernoulli_rng(phi[s,n]);
        n_obs_pred[s,n,j] = gs[j] * neg_binomial_2_rng(lambda[s,n,j], od[s]) * bernoulli_rng(phi[s,n]);
      }
  ...
  for (s in 1:S) {
    ...
    for (i in 1:npc) {
      pred[s,i] = neg_binomial_2_rng(exp(X_pred_psi[i,] * beta_psi[s] + eps_bioregion[s, pred_bioreg[i]]), od[s]) * prop_pred[i] * av_gs[s] * bernoulli_rng(inv_logit(f[s, prediction_row[i], prediction_col[i]]));
