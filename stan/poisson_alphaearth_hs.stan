functions {
  ...
  return out;
    ...
    /**
   * Compute the weighted mean of group sizes adjusted by detection probability.
   *
   * This function calculates a weighted mean of group sizes (`gs`) using
   * frequency counts (`freqs`) adjusted by inverse detection probabilities (`det_p`).
   * It is typically used when you want to account for detection bias in observed group sizes.
   *
   * @param n_gs   Number of group size categories
   * @param gs     Vector of group sizes (length n_gs)
   * @param freqs  Vector of frequency counts for each group size (length n_gs)
   * @param det_p  Vector of detection probabilities for each group size (length n_gs)
   * @return       A real value representing the weighted mean group size
   */
  real weighted_prob_mean(int n_gs, vector gs, vector freqs, vector det_p) {
    vector[n_gs] wts = freqs ./ det_p;  // weights = frequency adjusted by detectability
...
data {
  ...
  array[n_site, S] int<lower=1> n_max; // max for poisson RN
  ...
  real<lower=0> hs_df;            // slab degrees of freedom (nu), e.g., 1.0
  real<lower=0> hs_scale;         // slab scale (s), e.g., 1.0 or 2.0
  real<lower=0> hs_global_scale;  // global scale (tau0), e.g., 0.5 or based on p0 heuristic
...
for (j in 1:n_gs) {
  for (s in 1:S) {
    n_freqs[s,j] = 0;  // initialize
    for (i in 1:n_site) {
      n_freqs[s,j] += n_obs[i, j, s];
    }
  }
}
...
parameters {
 // abundance parameters
  ...
  // Intercept for the X_psi part (NOT regularized)
  array[S] real beta_psi_intercept;

  // Regularized Horseshoe latents for slopes (columns 2..m_psi)
  array[S] vector[m_psi - 1] z_psi;                 // standard normals
  array[S] vector<lower=0>[m_psi - 1] lambda_psi;   // local half-Cauchy
  array[S] real<lower=0> tau_psi;                   // global half-Cauchy
  array[S] real<lower=0> c2_psi;                    // slab variance (c^2)
  // RN intercept
  array[S] vector[1] beta_int_rn;
  vector[det_ncb] beta_det;
  array[S] real beta_occ;
  real log_theta;
...
transformed parameters {
  ...
  array[S] vector[n_site] log_lambda_psi_rn;
  // eps group size
...
for(n in 1:n_site) {
  ...
  log_lambda_psi[s,n] = X_psi[n,] * beta_psi[s];
  log_lambda_psi_rn[s,n] = X_psi[n,] * append_row(beta_int_rn[s] , beta_psi[s, 2:m_psi]);
...
model {
  ...
  // Intercept (NOT regularized). Use any weak prior you prefer.
beta_psi_intercept ~ normal(0, 3);   // tighter than 5
beta_int_rn[,1]    ~ normal(0, 3);   // RN intercept too

  // Regularized Horseshoe for slopes (columns 2..m_psi)
  for (s in 1:S) {
    z_psi[s]      ~ normal(0, 1);                   // standard normals
    lambda_psi[s] ~ cauchy(0, 1);                   // half-Cauchy via <lower=0>
    tau_psi[s]    ~ cauchy(0, hs_global_scale);     // half-Cauchy global
    c2_psi[s]     ~ inv_gamma(0.5 * hs_df, 0.5 * hs_df * square(hs_scale)); // slab c^2
  }
...
for(i in 1:npc) {
  real eta = X_pred_psi[i,] * beta_psi[s] + log(prop_pred[i]) + log(av_gs[s]);
  real eta_safe = fmin(eta, 20.7);         // safety margin below 20.794...
  pred[s,i] = poisson_log_rng(eta_safe);   // or: pred[s,i] = exp(eta_safe); // mean, no RNG  if(pred[s,i] > max(N_site[s,])) {
