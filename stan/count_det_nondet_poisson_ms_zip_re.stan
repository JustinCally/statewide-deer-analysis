...
parameters {
 // abundance parameters
  ...
  // site sigma RE
  real<lower=0> site_sd_sigma;
  vector[n_site] site_raw_sigma;
  // bioregion RE ZIP
...
for(n in 1:n_site) {
  eps_site_sigma[n] = site_sd_sigma * site_raw_sigma[n];
  log_sigma[n] = det_model_matrix[n,] * beta_det + eps_site_sigma[n];
...
model {
  ...
  site_sd_sigma ~ normal(0,0.5);
  site_raw_sigma ~ normal(0,1);
