...
transformed parameters {
  ...
  //   eps_bioregion[b] = bioregion_mu[b] + bioregion_sd * bioregion_raw[b];
  // }
...
for(i in 1:npc) {
  pred[i] = poisson_rng(exp(X_pred_psi[i,] * beta_psi)) * prop_pred[i]; //offset
}
Nhat = sum(pred);
}
