...
parameters {
  ...
  array[S] real beta_occ;                   // occupancy intercepts (for psi)
...
transformed parameters {
  ...
  array[S] vector[n_site] eta;             // X*beta
  array[S] vector[n_site] log_lambda_psi;  // Poisson log-mean (base)
  ...
  for (s in 1:S) {
    for (n in 1:n_site) {
      // X_psi first column is intercept, but we still write it as a full dot product
      eta[s,n]            = beta_occ[s] + eps_bioregion[s, site_bioreg[n]];
      log_lambda_psi[s,n] = eta[s,n];  // base Poisson log-mean
...
model {
  ...
  // ZIP for counts: mixture on each (s,n,j) using log_lambda
  for (s in 1:S) {
    for (n in 1:n_site) {
      ...
      for (j in 1:n_gs) {
        int k = n_obs[n, j, s];
        if (k == 0) {
          real pois0 = poisson_log_lpmf(0 | log_lambda[s,n,j]);
          target += log_sum_exp(log_psi + pois0, log1m_psi);
        } else {
          target += log_psi + poisson_log_lpmf(k | log_lambda[s,n,j]);
        }
      }
    }
  }
...
generated quantities {
  ...
  for (n in 1:n_site) {
    for (j in 1:n_gs) {
      ...
      for (s in 1:S) {
        ...
        if (k == 0) {
          real pois0 = poisson_log_lpmf(0 | log_lambda[s,n,j]);
          log_lik2[s, n, j] = log_mix( psi[s,n], pois0, 0 );
        } else {
          log_lik2[s, n, j] =
            log(psi[s,n]) +
            poisson_log_lpmf(k | log_lambda[s,n,j]);
        }
      }
  ...
  // per-species, per-site log-liks (if you want them)
  for (s in 1:S) {
    for (n in 1:n_site) {
  ...
  // Posterior predictive counts at sites
  for (n in 1:n_site) {
    for (s in 1:S) {
      real z_draw = bernoulli_rng( psi[s, n] );
      for (j in 1:n_gs) {
        real log_lambda_true = log_lambda_psi[s,n] + log(eps_ngs[s,n,j]);
        real lambda_pred     = exp(log_lambda[s,n,j]);

        if (z_draw == 1) {
          n_obs_true[s, n, j] =
            gs[j] * poisson_log_rng( log_lambda_true );
          n_obs_pred[s, n, j] =
            gs[j] * poisson_rng( lambda_pred );
        } else {
          n_obs_true[s, n, j] = 0;
          n_obs_pred[s, n, j] = 0;
        }
  ...
  for (s in 1:S) {
    ...
    for (i in 1:npc) {
      if (prop_pred[i] <= 0) {
        ...
        real eta_pred = beta_occ[s] + eps_bioregion[s, pred_bioreg[i]]
                        + log(prop_pred[i])
                        + log(av_gs[s]);
        real psi_pred =
