...
if (n_survey[n] > 0) {
  vector[n_max[n,s]] lp;
    if(any_seen[s,n] == 0){ // not seen
      array[2] real lpx;
      lpx[1] = log_sum_exp(log1m(phi[s,n]), log(phi[s,n]) + neg_binomial_2_log_lpmf(0 | log_lambda_psi[s,n], od[s]));
      lpx[2] = log(phi[s,n]) + neg_binomial_2_log_lpmf(1 | log_lambda_psi[s,n], od[s]) + bernoulli_lpmf(y2[s,start_idx[n]:end_idx[n]] | r[start_idx[n]:end_idx[n]]);
      lp[1] = log_sum_exp(lpx);
    } else {
      lp[1] = log(phi[s,n]) + neg_binomial_2_log_lpmf(1 | log_lambda_psi[s,n], od[s]) + bernoulli_lpmf(y2[s,start_idx[n]:end_idx[n]] | r[start_idx[n]:end_idx[n]]);
    }
    for (k in 2:n_max[n,s]){
      lp[k] = log(phi[s,n]) + neg_binomial_2_log_lpmf(k | log_lambda_psi[s,n], od[s]) + bernoulli_lpmf(y2[s,start_idx[n]:end_idx[n]] | 1-(1-r[start_idx[n]:end_idx[n]])^k);
    }
  ...
  log_lik2_site[n, j] = log_sum_exp(log_lik2[,n,j]);
    ...
    if(any_seen[s,n] == 0) {
      if(bernoulli_rng(phi[s,n])==0)
         N_site[s,n] = 0;
      log_p_unobs = log(phi[s,n]) + neg_binomial_2_log_lpmf(0 | log_lambda_psi[s,n], od[s]);
...
for(i in 1:npc) {
  ...
  pred[s,i] = neg_binomial_2_log_rng(X_pred_psi[i,] * beta_psi[s], od[s]) * prop_pred[i] * av_gs[s]; //offset
  } else {
    pred[s,i] = 0;
  }
