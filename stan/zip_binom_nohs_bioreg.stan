...
parameters {
  ...
  real<lower=0> sigma_beta;  // scale (controls shrinkage)
...
model {
  // priors
  // prior on the scale (global shrinkage)
  sigma_beta ~ exponential(1);     // tweak rate as needed
  for (s in 1:S) {
    to_vector(eps_raw[s,,]) ~ std_normal();
    grp_sd[s] ~ normal(0, 1);
    zeta[s,]  ~ normal(0, 2);
    // Laplace prior
    beta_psi[s,1] ~ normal(0,2);
