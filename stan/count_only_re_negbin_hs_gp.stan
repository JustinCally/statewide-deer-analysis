...
data {
  ...
  // gp process data
  int<lower = 1, upper = n_site> m;
  matrix[m, m] D_star;
  matrix[n_site, m] D_site_star;
...
transformed parameters {
  ...
  vector[m_psi] beta_psi;
    // gp transformed parameters
  vector[n_site] w;
  vector[n_site] sigma_e_tilde;
  matrix[m, m] Cstar;
  vector[m] w_star;
  matrix[m, m] inv_Cstar;
  matrix[n_site, m] C_site_star;
  matrix[n_site, m] C_ss_inv_Cstar;
  real eta_sq;
  real sig_sq;
  ...
  eta_sq = pow(eta, 2);
  sig_sq = pow(sigma_gp, 2);

    // latent gp at knots
  for (i in 1:(m-1)) {
    for (j in (i + 1):m) {
      Cstar[i, j] = eta_sq * exp(-D_star[i, j] * phi_gp);
      Cstar[j, i] = Cstar[i, j];
    }
  }
  ...
  C_site_star = eta_sq * exp(-D_site_star * phi_gp);
